...
public class NewsController : ControllerBase
{
    ...
    {
        Console.WriteLine($"SchoolChannelID: {request?.SchoolChannelID}");
        Console.WriteLine($"Title: {request?.Title}");
        Console.WriteLine($"Content: {request?.Content}");
        Console.WriteLine($"Image Count: {request?.ImageFiles?.Count ?? 0}");
        Console.WriteLine($"FollowerMode: {request?.FollowerMode}");
